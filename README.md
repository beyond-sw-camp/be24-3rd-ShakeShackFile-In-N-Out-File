...
<details>
<summary>Backend 및 프론트 설정 </summary>
<a href="https://github.com/beyond-sw-camp/be24-2nd-ShakeShackFile-In-N-Out-File" target="_blank"> 프론트 git 주소 </a>
<p>먼저 프론트 코드 및 현재 git 코드들이 필요합니다.</p>
먼저 프론트 코드의 서버를 실행 해야합니다. nginx로 하거나 IDE로 실행하셔도 됩니다. IDE의 경우에는 괜찮지만
nginx로 설정하실 경우
...
이 설정을 해주셔야 합니다.
리눅스를 기준으로 설명해드리자면 /etc/nginx/conf.d/default.conf로 설정해주시면 됩니다.
위의 경우에는 wss인 https로 연결할 때이며 http이면 ws로 해주셔야 합니다.
