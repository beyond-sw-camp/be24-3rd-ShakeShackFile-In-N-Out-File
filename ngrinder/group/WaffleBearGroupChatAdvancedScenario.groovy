...
class WaffleBearGroupChatAdvancedScenario {
    ...
    static void beforeProcess() {
        ...
        loginTest = new GTest(1, "groupchat-advanced-login")
        endpointTest = new GTest(2, "groupchat-advanced-endpoint")
    ...
    @Test
    void testRelationshipsAndOverview() {
        if (skipNetwork) {
            return
        }

        HTTPResponse relationships = get(endpointRequest, "/group/relationships")
        assertStatus(relationships, "GET /group/relationships", [200])

        HTTPResponse overview = get(endpointRequest, "/group/overview")
        assertStatus(overview, "GET /group/overview", [200])
    }
    ...
    void testCreateAndDeleteGroup() {
        ...
        HTTPResponse createGroupResponse = post(endpointRequest, "/group/groups", [name: "ngrinder-adv-group-${suffix}"])
        assertStatus(createGroupResponse, "POST /group/groups", [200])

        Object parsedGroup = parseJson(createGroupResponse.getText())
        Object groupBody = unwrapApiBody(parsedGroup)
        Long groupId = asLong(groupBody instanceof Map ? ((Map) groupBody).get("groupId") : null)

        if (groupId != null) {
            HTTPResponse deleteResponse = delete(endpointRequest, "/group/groups/${groupId}")
            assertStatus(deleteResponse, "DELETE /group/groups/{groupId}", [200])
        }
    }

    @Test
    void testChatRoomPagination() {
        if (skipNetwork) {
            return
        }

        HTTPResponse firstPage = get(endpointRequest, "/chatRoom/list", [page: 0, size: 5])
        assertStatus(firstPage, "GET /chatRoom/list?page=0", [200])

        HTTPResponse secondPage = get(endpointRequest, "/chatRoom/list", [page: 1, size: 5])
        assertStatus(secondPage, "GET /chatRoom/list?page=1", [200])
    }
    ...
    private HTTPResponse delete(HTTPRequest request, String path) {
        request.setHeaders(authJsonHeaders())
        return request.DELETE(buildUrl(path))
    }
