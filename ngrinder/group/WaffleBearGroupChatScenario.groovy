...
class WaffleBearGroupChatScenario {
    ...
    static void beforeProcess() {
        ...
        loginTest = new GTest(1, "groupchat-login")
        endpointTest = new GTest(2, "groupchat-scenario")
    ...
    void runScenario() {
        ...
        HTTPResponse relationshipsResponse = get(endpointRequest, "/group/relationships")
        assertStatus(relationshipsResponse, "GET /group/relationships", [200])

        HTTPResponse overviewResponse = get(endpointRequest, "/group/overview")
        assertStatus(overviewResponse, "GET /group/overview", [200])

        String suffix = UUID.randomUUID().toString().substring(0, 8)
        HTTPResponse createGroupResponse = post(endpointRequest, "/group/groups", [name: "ngrinder-group-${suffix}"])
        ...
        if (groupId != null) {
            HTTPResponse deleteGroupResponse = delete(endpointRequest, "/group/groups/${groupId}")
            assertStatus(deleteGroupResponse, "DELETE /group/groups/{groupId}", [200])
        }

        HTTPResponse chatRoomListResponse = get(endpointRequest, "/chatRoom/list", [page: 0, size: 5])
        assertStatus(chatRoomListResponse, "GET /chatRoom/list", [200])
    }
