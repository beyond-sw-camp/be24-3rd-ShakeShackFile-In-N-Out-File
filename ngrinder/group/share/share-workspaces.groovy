...
class TestRunner {
    protected static String baseUrl = System.getProperty('baseUrl', 'http://172.18.0.5:8080/group') // http://172.18.0.5:8080/group
    ...
    protected String workspaceFullUrl(String path) {
        if (path == null || path.isBlank()) {
            return baseUrl
        }
        if (path.startsWith('http://') || path.startsWith('https://')) {
    ...
    protected List<Map> workspaceListItems() {
        def response = request.GET(workspaceFullUrl('/workspace/list').toString(), [:], authHeaders())
    ...
    void test() {
        def response = postJson('/group/share/workspaces', [
                workspaceId: resolveWorkspaceIdx(true),
