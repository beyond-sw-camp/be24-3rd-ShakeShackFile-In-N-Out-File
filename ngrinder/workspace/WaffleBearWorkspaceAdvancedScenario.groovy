...
class WaffleBearWorkspaceAdvancedScenario {
    ...
    static void beforeProcess() {
        ...
        loginTest = new GTest(1, "workspace-advanced-login")
        endpointTest = new GTest(2, "workspace-advanced-endpoint")
    ...
    @Test
    void testListSaveDeleteFlow() {
        if (skipNetwork) {
            return
        }

        HTTPResponse listBefore = get(endpointRequest, "/workspace/list")
        assertStatus(listBefore, "GET /workspace/list (before)", [200])

        Map created = createWorkspace("advanced-list-save")

        HTTPResponse listAfter = get(endpointRequest, "/workspace/list")
        assertStatus(listAfter, "GET /workspace/list (after)", [200])

        deleteWorkspaceSilently(created.idx as Long)
    }

    @Test
    void testReadAndByUuidFlow() {
        if (skipNetwork) {
            return
        }

        Map created = createWorkspace("advanced-read-uuid")
        try {
            HTTPResponse readResponse = get(endpointRequest, "/workspace/read/${created.idx}")
            assertStatus(readResponse, "GET /workspace/read/{idx}", [200])

            String uuid = created.uuid as String
            if (uuid != null && !uuid.trim().isEmpty()) {
                HTTPResponse byUuidResponse = get(endpointRequest, "/workspace/by-uuid/${uuid}")
                assertStatus(byUuidResponse, "GET /workspace/by-uuid/{uuid}", [200])
            }
        } finally {
            deleteWorkspaceSilently(created.idx as Long)
        }
    ...
    @Test
    void testShareRoleFlow() {
        if (skipNetwork) {
            return
        }

        Map created = createWorkspace("advanced-share-role")
        try {
            HTTPResponse shareResponse = post(endpointRequest, "/workspace/isShared/${created.idx}", [type: true, status: "Public"])
            assertStatus(shareResponse, "POST /workspace/isShared/{idx}", [200])

            HTTPResponse loadRoleResponse = get(endpointRequest, "/workspace/loadRole/${created.idx}")
            assertStatus(loadRoleResponse, "GET /workspace/loadRole/{idx}", [200])

            HTTPResponse saveRoleResponse = post(endpointRequest, "/workspace/saveRole/${created.idx}", [:])
            assertStatus(saveRoleResponse, "POST /workspace/saveRole/{idx}", [200])
        } finally {
            deleteWorkspaceSilently(created.idx as Long)
        }
    }
    ...
    private Map createWorkspace(String tag) {
        ...
        Map payload = [
                ...
                contents: "workspace advanced scenario ${tag} @ " + System.currentTimeMillis()
        ]
