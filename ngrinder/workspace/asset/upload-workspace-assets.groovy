...
class TestRunner {
    ...
    protected Long resolveWorkspaceIdx(boolean writeRequired = false) {
        String configured = prop('workspaceId', '')
        ...
        List<Map> filtered = writeRequired
                ? items.findAll { Map item -> (item.level ?: '').toString() != 'READ' }
                : items

        assert filtered != null && !filtered.isEmpty() : (
                writeRequired ? 'No writable workspace is available for this user.' :
    ...
    void test() {
        def payload = multipartPayload([
                [name: 'files', filename: prop('assetFile1', uniqueValue('asset-1') + '.png'), contentType: 'image/png', bytes: samplePngBytes()],
                [name: 'files', filename: prop('assetFile2', uniqueValue('asset-2') + '.png'), contentType: 'image/png', bytes: samplePngBytes()]
        ])
        def response = request.POST(
                fullUrl('/workspace/' + resolveWorkspaceIdx(true) + '/assets'),
                (byte[]) payload.bytes,
