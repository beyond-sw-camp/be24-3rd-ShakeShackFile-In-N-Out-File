...
class TestRunner {
    ...
    void test() {
        def payload = multipartPayload([
                [name: 'image', filename: prop('editorImageFile', uniqueValue('editor') + '.png'), contentType: 'image/png', bytes: samplePngBytes()]
        ])
        def response = request.POST(
                fullUrl('/workspace/' + resolveWorkspaceIdx(true) + '/assets/editorjs'),
                (byte[]) payload.bytes,
