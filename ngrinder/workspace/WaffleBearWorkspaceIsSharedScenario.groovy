...
class WaffleBearWorkspaceIsSharedScenario {
    ...
    static void beforeProcess() {
        ...
        loginTest = new GTest(1, "workspace-isShared-login")
        endpointTest = new GTest(2, "workspace-isShared-endpoint")
    ...
    @Test
    void testWorkspaceIsShared() {
        if (skipNetwork) {
            return
        }
        Map created = createWorkspace("is-shared")
        try {
            Map payload = [
                    type  : true,
                    status: "Public"
            ]
            HTTPResponse response = post(endpointRequest, "/workspace/isShared/${created.idx}", payload)
