...
class WaffleBearWorkspaceByUuidScenario {
    ...
    static void beforeProcess() {
        ...
        loginTest = new GTest(1, "workspace-by-uuid-login")
        endpointTest = new GTest(2, "workspace-by-uuid-endpoint")
    ...
    @Test
    void testWorkspaceByUuid() {
        if (skipNetwork) {
            return
        }
        Map created = createWorkspace("by-uuid")
        try {
            String uuid = created.uuid as String
            if (!uuid) {
                throw new IllegalStateException("Workspace uuid was not found in save response.")
            }
            HTTPResponse response = get(endpointRequest, "/workspace/by-uuid/${uuid}")
