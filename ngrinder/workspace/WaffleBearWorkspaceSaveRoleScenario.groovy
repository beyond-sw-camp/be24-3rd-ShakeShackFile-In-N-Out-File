...
class WaffleBearWorkspaceSaveRoleScenario {
    ...
    static void beforeProcess() {
        ...
        loginTest = new GTest(1, "workspace-saveRole-login")
        endpointTest = new GTest(2, "workspace-saveRole-endpoint")
    ...
    @Test
    void testWorkspaceSaveRole() {
        if (skipNetwork) {
            return
        }
        Map created = createWorkspace("save-role")
        try {
            HTTPResponse response = post(endpointRequest, "/workspace/saveRole/${created.idx}", [:])
