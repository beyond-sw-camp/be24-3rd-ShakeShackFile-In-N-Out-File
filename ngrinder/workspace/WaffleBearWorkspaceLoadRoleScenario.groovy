...
class WaffleBearWorkspaceLoadRoleScenario {
    ...
    static void beforeProcess() {
        ...
        loginTest = new GTest(1, "workspace-loadRole-login")
        endpointTest = new GTest(2, "workspace-loadRole-endpoint")
    ...
    @Test
    void testWorkspaceLoadRole() {
        if (skipNetwork) {
            return
        }
        Map created = createWorkspace("load-role")
        try {
            HTTPResponse response = get(endpointRequest, "/workspace/loadRole/${created.idx}")
