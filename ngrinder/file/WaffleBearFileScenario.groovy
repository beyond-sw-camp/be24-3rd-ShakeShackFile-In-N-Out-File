...
class WaffleBearFileScenario {
    ...
    static void beforeProcess() {
        ...
        loginTest = new GTest(1, "file-login")
        endpointTest = new GTest(2, "file-scenario")
    ...
    void runScenario() {
        ...
        assertStatus(listBefore, "GET /file/list", [200])

        String suffix = UUID.randomUUID().toString().substring(0, 8)
        HTTPResponse folderResponse = post(endpointRequest, "/file/folder", [folderName: "ngrinder-folder-${suffix}", parentId: null])
        assertStatus(folderResponse, "POST /file/folder", [200])

        Object folderBody = unwrapApiBody(parseJson(folderResponse.getText()))
        Long folderId = asLong(folderBody instanceof Map ? ((Map) folderBody).get("idx") : null)

        String fileName = "ngrinder-${suffix}.txt"
        byte[] payload = ("nGrinder upload test at " + System.currentTimeMillis()).getBytes(StandardCharsets.UTF_8)
        ...
        HTTPResponse initResponse = post(endpointRequest, "/file/upload", initRequestBody)
        ...
        HTTPResponse completeResponse = post(endpointRequest, "/file/upload/complete", completeRequestBody)
        assertStatus(completeResponse, "POST /file/upload/complete", [200])

        String abortFileName = "ngrinder-abort-${suffix}.txt"
        def abortInitRequestBody = [[
                fileOriginName: abortFileName,
                fileFormat    : "txt",
                fileSize      : payload.length,
                contentType   : "text/plain",
                parentId      : folderId,
                relativePath  : abortFileName,
                lastModified  : System.currentTimeMillis()
        ]]

        HTTPResponse abortInitResponse = post(endpointRequest, "/file/upload", abortInitRequestBody)
        assertStatus(abortInitResponse, "POST /file/upload (abort case)", [200])

        List<Map> abortChunks = normalizeChunkList(parseJson(abortInitResponse.getText()))
        if (!abortChunks.isEmpty()) {
            def abortRequestBody = [
                    finalObjectKey : (abortChunks[0].finalObjectKey ?: abortChunks[0].objectKey),
                    chunkObjectKeys: abortChunks.collect { it.objectKey as String }.findAll { it != null && !it.isEmpty() }
            ]
            HTTPResponse abortResponse = post(endpointRequest, "/file/upload/abort", abortRequestBody)
            assertStatus(abortResponse, "POST /file/upload/abort", [200])
        }

        HTTPResponse listAfter = get(endpointRequest, "/file/list")
        assertStatus(listAfter, "GET /file/list (after upload)", [200])
    }
