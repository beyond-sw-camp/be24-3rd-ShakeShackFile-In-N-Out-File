...
class TestRunner {
    protected static String baseUrl = System.getProperty('baseUrl', 'http://172.18.0.5:8080/chat') // http://172.18.0.5:8080/chat
    ...
    void test() {
        def payload = multipartPayload([
                [name: 'file', filename: prop('chatFileName', uniqueValue('chat') + '.txt'), contentType: 'text/plain', bytes: textBytes(prop('chatFileContent', uniqueValue('ngrinder-chat-attachment')))]
        ])
